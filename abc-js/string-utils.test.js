...
export function repeatTests() {
  ...
  // resultValue = repeat(complexText, 3);
  ...
  // let resultValue = repeat(complexText, 1);
  ...
  // assertThrow('второй параметр приводится в число', repeat('a', '3'), 'aaa');

  // let errCb = () => repeat('a', -1);
  // assertToBe('отрицательное число повторении вызывает исключение', errCb, 'repeat count must be positive value or zero');
  // errCb = () => repeat();
